#!/bin/bash
# setup-all-claude-code.sh - Setup all procedures (wizard + memory) to ~/.claude/commands/
# Uses a manifest to track installed files and clean up stale commands on re-run.
#
# Usage: ./control-files/procedures/setup-scripts/setup-all-claude-code.sh
...
echo "=== Setup All Procedures as Slash Commands ==="
echo ""
