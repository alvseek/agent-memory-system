...
Graduate a **consenting** project's orientation map + *structural* project-context OUT of the central `@agent-memory` store and INTO the project's own repository (`.agents/`). One-time, forward-only. See [ADR-005](//@agent-memory/docs/adr/2026-07-06-project-local-memory-externalization.md).
...
| Localizes → `.agents/` | Stays central (fleet-private) |
|---|---|
...
3. **Guards** (STOP + report if any fail):
   ...
   - Central map already has `home: project` → *"[PROJECT] is already localized (`.agents/orientation-map.md`)."* Exit.

> `--status` mode stops here: report localized (map `home: project`) or central-only, then exit.

## Step 2: Consent gate + `AGENTS.md`

1. **Confirm consent** — one explicit confirmation from [USER-NAME]: *"Localize [PROJECT] into its own repo? This moves the map + structural context into `<project-root>/.agents/` and leaves a central stub. Confirm the project is agreed to host AI artifacts."* Wait.
2. **Root `AGENTS.md`**:
...
3. **Append the prose pointer** (never a boolean flag — the standard is prose sections):
   ...
   ```markdown
   ## Agent Orientation
   This repo maintains a structured orientation map at `.agents/orientation-map.md`.
...
3. **Present the full classification for confirmation BEFORE any move** — this is the safety pause:
   ...
   Localization plan for [PROJECT]:
     MOVE → .agents/context/:   [file] — [why structural]
...
## Step 4: Create `.agents/` layout

- `mkdir -p <project-root>/.agents/context`

## Step 5: Move + rebase the orientation map

1. **Copy** central map → `<project-root>/.agents/orientation-map.md`.
...
3. **Rebase paths** (two different bases — don't conflate):
   ...
   - **`child_map` sub-links** are relative to the **map file's folder**. The map now lives at `.agents/`, so rewrite the old central-context form (`../../../[sub]/orientation-map.md`) to `.agents/`-relative: `../[sub]/orientation-map.md` (one `..` climbs from `.agents/` to the repo root, then descends into the sub-project). This keeps `/map-orientation` Load Mode L2 (which follows `child_map` relative to the parent map's location) working unchanged.
...
1. For each **localize**-classified file: copy → `<project-root>/.agents/context/[file]`, then delete the central original.
2. **In-project index**: create `<project-root>/.agents/context/context-index.md` (header `# [PROJECT] Project Context`) with the moved entries.
...
home: project
localized_path: .agents/orientation-map.md
...
`.agents/orientation-map.md` (source of truth). This central file is a stub;
resolution keys off the project root (cwd) — see Localized Home Resolution.

### `.agents/orientation-map.md` (in-project)
...
- Commit `.agents/` + `AGENTS.md` in the **project** repo.
...
3. **Read central map frontmatter**:
   - `home: project` present →
     ...
     - `map_path` = `<project_root>/<localized_path>` (default `.agents/orientation-map.md`)
     - `context_home` = `<project_root>/.agents/context/`
