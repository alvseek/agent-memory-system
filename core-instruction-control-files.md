...
18. **Todoist Tasks Available** (optional): If the Todoist MCP is connected AND `shared-memory/integrations/todoist.md` exists AND the current working directory matches a Todoist project per the mapping table in that doc, query active tasks filtered by `@agent-[my-domain]` and `@waiting-human` and briefly report counts: e.g., "Todoist (#invintiry): 3 tasks tagged `@agent-software-architect`, 1 `@waiting-human`." If Todoist MCP is not configured, or no matching Todoist project exists for the current directory, skip silently. **Never** attempt to read raw API tokens from anywhere; OAuth-only. See [integrations/todoist.md](../shared-memory/integrations/todoist.md) for the full convention.
