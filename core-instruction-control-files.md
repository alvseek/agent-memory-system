...
3. **Load recent context (project-scoped)**: from the `[Recent Context Episodes]` index (already loaded), load the latest episodic file whose filename or summary contains the current project name. Fallback: absolute latest entry, marked as not project-specific. In parallel, attempt both `knowledge-base/[PROJECT-NAME]/context-index.md` (private) and `shared-memory/[PROJECT-NAME]/context/context-index.md` (shared) — silently skip whichever is missing.
