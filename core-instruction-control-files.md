...
3. **Load recent context (project-scoped)**: determine the current project (from cwd) and apply [Localized Home Resolution](procedures/localize-context.md#localized-home-resolution) once (it resolves all lanes off `home: project` in the central map `shared-memory/[PROJECT-NAME]/context/orientation-map.md`; `<project-root>` = cwd):
    - **Episodic** ([ADR-011](docs/adr/2026-07-21-localized-episodic-index-repo-authoritative.md)): find the latest entry for this project, then read it —
      - **Central** (not localized): from the already-loaded `[Recent Context Episodes]` index find the latest entry whose filename or summary contains the project name (fallback: absolute latest entry, marked as not project-specific) → read the episode from `agent-[domain]/episodes/`.
      - **Localized** (`home: project`): the central index holds only a bounded `## Localized Projects` pointer (a fleet-trace, **not** the read source). Read the repo index `SESSION_DIR/index.md` (`<project-root>/.agents/session/index.md`), take its **top** (newest-first) entry → read that theme file from `SESSION_DIR`. **Reachability guard**: if `.agents/session/` is absent at cwd → report *"[project] is localized but not checked out here"* + skip open items.
