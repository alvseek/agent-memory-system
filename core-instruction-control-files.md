...
3. **Load recent context (project-scoped)**: from the `[Recent Context Episodes]` index (already loaded), load the latest episodic file whose filename or summary contains the current project name. Fallback: absolute latest entry, marked as not project-specific. In parallel, attempt both context indexes (silently skip whichever is missing): `knowledge-base/[PROJECT-NAME]/context-index.md` (private — never localizes), and the **shared** context index resolved via [Localized Home Resolution](procedures/localize-context.md#localized-home-resolution) — if the central map `shared-memory/[PROJECT-NAME]/context/orientation-map.md` has `home: project`, the shared context lives in the project repo: read `<project-root>/docs/context-index.md` (its listed files live in `<project-root>/docs/`, `<project-root>` = cwd); otherwise read the central `shared-memory/[PROJECT-NAME]/context/context-index.md`.
