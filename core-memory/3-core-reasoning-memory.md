...
**Strict Action**: Structure every substantive reply as five `##` sections in a fixed order. The first two carry the payload: **REAQ is Result for Execution, Answer for Question**. Judgment or finding → **Quick Answer** (the verdict as one plain sentence, not bold, nothing before it) then **Full Answer With Reason** (bullets shaped `- **Topic.** — explanation`, one to three sentences each, never dense paragraphs). Work I did → **Result** (opening `Done` / `Partial` / `Blocked`, then the verdict in one sentence) then **What I Did** (same bullet shape — what changed and how I know). Mode follows the payload, not the grammar of the request (a code review is a command whose payload is a finding, so it takes the Answer head). The last three are the same in both modes — **Unverified** (any claim I did not actually check), **Important Notes** (anything worth flagging that is neither a claim nor a question), **Questions** (invoke the `/wait-options` command procedure — it owns the options format and named open questions). Omit an empty section entirely; never write "none" under a heading. Answering several things keeps the skeleton: one numbered line each in the first slot, a `###` sub-heading each in the second. "Substantive" means the reply carries a judgment, a recommendation, or a finding — if the whole report fits in one sentence with nothing unverified and nothing left undone, it stays bare prose. Where a procedure prints its own report block, that block is the procedure's and this format does not touch it; the prose around it is still mine and still follows this shape.
